...
%done=load('undone1_task3_1d.txt');
midpath='task3_1d_';
...
partint=1;
part_total=2;
%outmat3=[dim*ones(part_total,1) zeros(part_total,5)];
outmat3=[];
...
for kk=1:L
    ...
    outmat3=[dim alpha.result_vector];    
    filename2=sprintf('%d%s',ind,'.txt');
    dlmwrite(fullfile(destpath,filename2),outmat3,'delimiter','\t');
...
function alpha = get_alpha(theta,model_no)
  ...
  elseif ismember(model_no,[1]) %Use if ctrw is "joined"
    alpha = theta{2}{2}{2}{1}.meta(1);
  elseif ismember(model_no,[2 4])
    alpha= theta{2}{2}(1);
  elseif ismember(model_no,[3]) % Disc
    alpha= theta{2}{2}(1); %Use when FBM/hmm_levy included
  end
end
